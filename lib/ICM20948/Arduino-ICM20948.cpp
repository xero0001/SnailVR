...
void ArduinoICM20948::check_rc(int rc, const char *msg_context)
{
  if (rc < 0)
  {
    Serial.print("ICM20948 ERROR, Code : ");
    Serial.print(rc);
    Serial.print(" ");
...
int ArduinoICM20948::init(ArduinoICM20948Settings settings)
{
  ...
  icm20948_serif.read_reg = [me](void *context, uint8_t reg, uint8_t *buf, uint32_t len) -> int
  { return me->idd_io_hal_read_reg(context, reg, buf, len); };
  icm20948_serif.write_reg = [me](void *context, uint8_t reg, const uint8_t *buf, uint32_t len) -> int
  { return me->idd_io_hal_write_reg(context, reg, buf, len); };
  ...
  else
  {
    ...
    Serial.print(F("Enabling DMP for IMU ID : "));
    Serial.print(settings.sensorId);

    rc = load_dmp3();

    if (rc != 0)
    {
      check_rc(rc, " Error DMP loading.");
    }
    else
    {
